...
async def generate_targeting_plan(role_id: str):
    ...
    await db.targeting_plans.find_one_and_delete({"role_id": role_id})
    result = await db.targeting_plans.insert_one(doc)
...
async def get_targeting_plan(role_id: str):
    db = get_db()
    doc = await db.targeting_plans.find_one({"role_id": role_id})
    if not doc:
